...
class FormatterTest < Test::Unit::TestCase
  ...
  context "#render_options" do
    ...
    should "set the return the default layout of 'pdf.pdf.erb'" do
      assert_equal RedmineChilliPDF::Formatter::DEFAULT_LAYOUT, @formatter.render_options[:layout]
    end

    should "set 'extended_wiki/show.pdf.html.erb' as the default template" do
      assert_equal RedmineChilliPDF::Formatter::DEFAULT_VIEW_TEMPLATE, @formatter.render_options[:template]
    end

    should "set the default page_size to be 'Letter'" do
      assert_equal RedmineChilliPDF::Formatter::DEFAULT_PAGE_SIZE, @formatter.render_options[:page_size]
    end

    should "set the default margins" do
      def_margin = RedmineChilliPDF::Formatter::DEFAULT_MARGIN
    ...
    context 'when footers are enabled' do
      ...
      should 'include footer options' do
        assert RedmineChilliPDF::Config.footer_enabled?
    ...
    context "replacing 'dynamic tokens' with expected content" do
      ...
      context "when a page title is supplied" do
        setup do
          ...
          @formatter = RedmineChilliPDF::Formatter.new('filename.pdf', @custom_title)
        end
      ...
      context "when a page title is not supplied" do
        should "substitues '{{page_title}}' with the default page title value '#{RedmineChilliPDF::Formatter::DEFAULT_PAGE_TITLE}'" do
          set_header_field_to "{{page_title}}"
          assert_equal RedmineChilliPDF::Formatter::DEFAULT_PAGE_TITLE, header_value
        end
      end
